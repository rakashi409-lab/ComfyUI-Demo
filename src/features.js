...
Object.assign(app, {
    version: '2.0.1', groups: [], nextGroupId: 1, selectedGroup: null,
    ...
    updateWorkflowTitle(){document.getElementById('workflow-name').textContent=this.workflowName;document.title=this.workflowName+' · ComfyUI';},
    loadPresetWorkflow() {
        if(this.isRunning || !this.confirmReplace())return;
    ...
    showResultOnNode(node,src){
        ...
        box.innerHTML=`<img src="${src}" alt="输出图像">`;box.querySelector('img').onload=()=>this.renderConnections();this.attachDownload(node);this.renderConnections();
    },
    ...
    exportWorkflow(){const data=this.snapshot();data.savedAt=new Date().toISOString();this.downloadBlob(new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),this.workflowName+'.json');this.notify('工作流已导出');},
...
Object.assign(app, {
    validateDocument(data) {
        ...
        const genImages=array(data.genImages||[],'图像',500).map(image);
    ...
    async openStorage(){
        ...
        this._db=await new Promise((resolve,reject)=>{
            if(!window.indexedDB){reject(Error('无法打开本地存储'));return;}
    ...
    showModels(){
        ...
        this.showDialog('模型库',`<p class="hint">管理当前工作流使用的模型名称。</p>${models.length?models.map(n=>`<label class="field">${esc(NODES[n.type].title)} #${n.id}<input data-model-id="${n.id}" value="${esc(n.widgets[0].value)}" ${this.isRunning?'disabled':''}></label>`).join(''):'请先添加加载器节点，或加载预设工作流。'}`);
    ...
    showGallery(){
        const images=[];for(const n of this.nodes){for(const w of n.widgets)if(w.type==='image'&&w.value)images.push({src:w.value,name:w.filename||'输入图片'});if(n.resultImage)images.push({src:n.resultImage,name:'输出图片'});}this.genImages.forEach((src,i)=>images.push({src,name:'图像 '+(i+1)}));
        this.showDialog('图像',images.length?'<div class="gallery">'+images.map((im,i)=>`<figure><img src="${im.src}" alt="${esc(im.name)}"><figcaption>${esc(im.name)}</figcaption><button class="tool" data-image-download="${i}">下载</button></figure>`).join('')+'</div>':'暂无图像。');
...
Object.assign(app, {
    ...
    async runWorkflow(){
        ...
        try{
            ...
            this.notify('执行完成');
        }catch(error){this._controller.abort();await Promise.allSettled(tasks);this.notify(error.name==='AbortError'?'执行已停止':'执行失败：'+error.message);}
        finally{
